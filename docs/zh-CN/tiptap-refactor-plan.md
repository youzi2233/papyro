...
| 维度 | 当前状态 | 目标状态 |
|------|---------|---------|
| 语言 | 混合 TypeScript，且截至 2026-05-13 `js/src/` 下仍有 41 个被跟踪的 `.js` 文件和 3 个被跟踪的 `.jsx` 文件 | TypeScript (.ts/.tsx) |
...
- JavaScript 存量：`js/src/` 下仍有 41 个被跟踪的 `.js` 文件和 3 个被跟踪的 `.jsx` 文件。它们是源码，不是生成物。它们分三类：必须迁移为 TypeScript 的核心 runtime/registry/Markdown sync；需要行为覆盖后类型化的 Papyro 特有 Markdown/媒体适配；以及应迁移为 TSX 或在官方组件完全接管后删除的旧 React 支撑文件。
...
- [ ] 将 `js/src/` 下剩余 41 个 `.js` 文件和 3 个 `.jsx` 文件迁移为 `.ts`/`.tsx`；若官方 TS/TSX 组件已经接管对应行为，则直接删除旧文件
