...
| 维度 | 当前状态 | 目标状态 |
|------|---------|---------|
| 语言 | 混合 TypeScript，且截至 2026-05-13 source pane 迁移后，`js/src/` 下仍有 11 个被跟踪的 `.js` 文件且不再有被跟踪的 `.jsx` 文件 | TypeScript (.ts/.tsx) |
...
- [ ] 将 `js/src/` 下剩余 11 个 `.js` 文件迁移为 `.ts`/`.tsx`，并保持无被跟踪 `.jsx` 文件；若官方 TS/TSX 组件已经接管对应行为，则直接删除旧文件
