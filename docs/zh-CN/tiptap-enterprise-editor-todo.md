...
- [ ] 默认隐藏可视句柄。只有在第一行或第一列附近有明确 hover 意图时展示行/列句柄。
  - 当前覆盖：行/列句柄默认保持隐藏；hover 第一列任意单元格时显示对应行句柄，hover 第一行任意单元格时显示对应列句柄。句柄仍位于可编辑单元格外侧，因此用户能获得接近官方的轴向操作入口，同时不影响正常单元格文本编辑。
...
- [ ] 多单元格框选后显示克制遮罩，并在选区边缘显示小操作触发点。
  ...
  - 当前打磨：React 渲染的表格 chrome 现在暴露共享的 `data-visible` 契约，并把隐藏控件从可访问树和焦点顺序中移除，避免快捷新增轨道和单元格触发点在隐藏态留下不可见焦点目标。
...
- [ ] 从细边缘句柄打开行/列操作菜单。
  ...
  - 当前打磨：行/列细句柄现在必须来自明确的边缘或 gutter hover 意图。鼠标普通停留在第一行/第一列单元格内容区时，表格会保持干净且可编辑。
