...
| `js/src/tiptap-i18n.ts` | Tiptap runtime chrome、命令菜单和可访问性文案的集中标签表。 |
| `js/src/tiptap-*.ts` | 聚焦的 Tiptap controller、command、Markdown handler 和 UI helper。 |
...
chrome 从 `js/src/tiptap-i18n.ts` 读取面向用户的标签。这样 Tiptap 组件状态仍
...
`.mn-math-inline` 与 `.mn-math-block` 占位符。`js/src/tiptap-math.ts`
...
- 窗口标题会跟随共享 app context 的当前语言更新。原生窗口图标继续从打包的 logo bytes 加载，共享 UI 内的品牌 logo 则通过内嵌 logo context 提供，避免次级窗口依赖磁盘上的 `/assets/logo.png` 才能显示品牌。
...
| 改主题 token 或 Markdown 视觉样式 | [theme-system.md](theme-system.md) | `assets/main.css`, `js/src/tiptap-*.ts`, `js/src/tiptap-react/` |
...
| 改 Hybrid 编辑 | `js/src/editor-runtime.ts`, `js/src/tiptap-*.ts` | `js/src/tiptap-react/`, `crates/editor/src/parser/blocks.rs` |
| 改粘贴/选区/IME | `js/src/tiptap-paste-controller.ts`, `js/src/tiptap-ui-primitives.ts` | `js/test/tiptap-*.test.ts` |
...
4. JS 在 `js/src/editor-runtime.ts` 或聚焦的 `js/src/tiptap-*.ts` controller 中处理新命令。
