...
- [ ] Resolve the remaining typecheck blockers before enabling the gate: global runtime/model debt in `editor-core.ts`, `editor-runtime.ts`, `editor-runtime-contract.ts`, `tiptap-react/runtime-model.ts`, `editor-runtime-protocol.ts`, `tiptap-react/runtime-context.tsx`, `editor-host-runtime.ts`, and `tiptap-block-move.ts`; runtime context/island boundaries, `tiptap-react/utils/floating.ts`, and `tiptap-react/mount-controller.tsx`; and the Papyro custom table action/Markdown layer in `tiptap-table.ts`
