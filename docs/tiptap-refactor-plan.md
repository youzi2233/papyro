...
|-----------|--------------|--------------|
| Language | Mixed TypeScript with 2 tracked `.js` files and no tracked `.jsx` files still under `js/src/` as of the 2026-05-13 block actions migration | TypeScript (.ts/.tsx) |
...
- JavaScript inventory: `js/src/` still contains 2 tracked `.js` files and no tracked `.jsx` files after the block actions migration. They are source files, not generated output. The remaining files are Papyro-specific Markdown/editor adapters and code block behavior that should be typed after behavior coverage or deleted when official TS/TSX components own the behavior.
...
- [ ] Migrate the remaining 2 `.js` files and no `.jsx` files under `js/src/` to `.ts`/`.tsx`, or delete them when an official TS/TSX component already owns the behavior
