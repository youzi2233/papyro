...
|-----------|--------------|--------------|
| Language | Mixed TypeScript with 25 tracked `.js` files and no tracked `.jsx` files still under `js/src/` as of the 2026-05-13 history command migration | TypeScript (.ts/.tsx) |
...
- [ ] Migrate the remaining 25 `.js` files and no `.jsx` files under `js/src/` to `.ts`/`.tsx`, or delete them when an official TS/TSX component already owns the behavior
