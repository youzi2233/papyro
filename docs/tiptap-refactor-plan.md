...
|-----------|--------------|--------------|
| Language | Mixed TypeScript with 34 tracked `.js` files and 3 tracked `.jsx` files still under `js/src/` as of 2026-05-13 | TypeScript (.ts/.tsx) |
...
- JavaScript inventory: `js/src/` still contains 34 tracked `.js` files and 3 tracked `.jsx` files. They are source files, not generated output. They fall into three buckets: Papyro-specific Markdown/media adapters that should be typed after behavior coverage, legacy React support files that should either migrate to TSX or be deleted once the official component path fully owns the UX, and a small number of runtime smoke/utility modules awaiting migration.
...
- [ ] Migrate the remaining 34 `.js` files and 3 `.jsx` files under `js/src/` to `.ts`/`.tsx`, or delete them when an official TS/TSX component already owns the behavior
