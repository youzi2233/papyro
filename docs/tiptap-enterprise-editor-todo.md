...
- [ ] Add a typed command model for insert, block action, inline format, table, and code block commands.
  ...
  - Current coverage: the visible official table-node cell, row, and column handle menus now consume the same Papyro table command model. The generated official duplicate paths re-export the Papyro-owned menu implementations, preventing hidden source drift between the mounted runtime path and copied UI source.
...
- [ ] Add cell action menu: merge, split, alignment, text color, background color, clear formatting, copy, delete contents.
  ...
  - Current coverage: the visible official cell handle menu now exposes Papyro's scoped commands for single-cell and range selections, including merge/split, copy, clear content, clear style, alignment, text color, and cell background. It no longer delegates to generic text/highlight `ColorMenu`, so table styling operates on selected cells instead of arbitrary text marks.
- [ ] Add row and column action menus from slim edge handles.
  ...
  - Current coverage: the visible official row/column handle menus now render the Papyro command model directly, including structure, content, style, and danger sections plus right-side style submenus. Runtime language is passed from the React island, and source-level tests guard against falling back to the generic official menu copy.
