...
- Already integrated: licensed `table-node` visible table chrome, official table handle extension boundary, `@tiptap/extension-drag-handle-react`, `@tiptap/extension-node-range`, official color/highlight package paths, the React island slot model, `slash-dropdown-menu`, `suggestion-menu`, `drag-context-menu`, `slash-command-trigger-button`, `turn-into-dropdown`, `duplicate-button`, `delete-node-button`, and `reset-all-formatting-button`.
- Next official candidates for local Markdown UX: `floating-element`, `move-node-button`, `link-popover`, `heading-dropdown-menu`, `list-dropdown-menu`, `mark-button`, `text-align-button`, `undo-redo-button`, `image-node-pro`, `image-align-button`, and `image-upload-button`.
...
- [ ] Add a typed command model for insert, block action, inline format, table, and code block commands.
  ...
  - Current decision: because the project now accepts the Tiptap Pro/Start licensed path, visible table interactions must stay owned by the generated official `table-node` components. Papyro table command models remain available for migration fallback and local Markdown commands, but they must not replace the official handle/menu interaction contract in the runtime path.
...
- [ ] Make the entire cell surface editable and focusable, not only a tiny center area.
  ...
  - Current fix: when an existing table range is collapsed back into editing through the official handle plugin, the plugin now restores a folded caret at the pointer location instead of selecting the whole paragraph inside the cell.
