...
- [ ] Remove visible handles by default. Show row/column handles only on intentional hover near the first row or first column.
  ...
  - Current architecture: non-menu table chrome now has a React renderer injected through `tableChromeRendererFactory`; the old DOM renderer remains as fallback while future row/column/cell affordance work moves into reusable React components.
