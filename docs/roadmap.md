...
- Do not copy non-open Tiptap UI Components such as Notion-like, table-node, drag-context-menu, or slash-dropdown-menu unless licensed CLI output is explicitly added to the project.
...
- [x] Tighten slash/`+` and block-action panel sizing again with a wider shell, smaller icon wells, and steadier row rhythm so the dropdowns read less like generic button lists.
...
- [x] Preserve the hovered `+` insertion command across selection/update refreshes, so table and callout detail panels stay on the command under the pointer instead of jumping back to the first item.
...
- [x] Wire the table context menu into the shared active-descendant primitive so hover, focus, roving tab index, and keyboard navigation follow the same contract as slash and block action panels.
